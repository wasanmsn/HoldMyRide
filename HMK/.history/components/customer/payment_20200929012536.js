...
function Payments ({navigation}){
    ...
    const  getPayments = async () => {
            try {
                const id = await AsyncStorage.getItem("ID")
                await firestore().collection('payment_History').where('buyer','==',firestore().doc('customer/'+id)).get().then(snapshot => {
                    if(!snapshot.empty){
                    ...
                    await firestore().collection('payment_History').where('payer','==',firestore().doc('customer/'+id)).get().then(snapshot => {
                        if(!snapshot.empty){
                            snapshot.forEach( async data => {            
                                setPayments(prev => [...prev,data.data()]) 
                            })
                ...
                payments.map((item,index) => 
                    ...
                    (<Card key={index} style={{marginVertical:15,flexDirection:'row',justifyContent:'space-between'}}   >
                        ...
                        <Card.Content>
                                ...
                                <Text style={{color: payment_type == 'ฝากรถ' ? 'red' : 'green',fontSize:15,alignSelf:'flex-end'}}>
                                    {payment_type == 'ฝากรถ' ? '-' : '+'} {currencyFormat(item.price)} บาท
                                </Text>
                        </Card.Content>
