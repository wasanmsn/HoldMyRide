        ...
        <View style={page.container}>
            ...
            <ScrollView style={page.box1}>
                ...
                vehis.map((item,index) => 
                    ...
                    (<Card key={item.Plate} style={{borderColor:'black',borderWidth:1,height:85,marginVertical:15,flexDirection:'row',justifyContent:'space-between'}} 
                    onPress={() => navigation.navigate('Detail',{item})} >
                        <Card.Title
                            title={`${item.Name} ${item.Plate}`}
                            subtitle={`เหลือเวลาอีก ${item.expiredate == null ? '': "เหลือเวลาอีก "+Math.floor((new Date(item.expiredate.seconds*1000).getTime() - new Date().getTime())/(1000*3600*24))+' วัน'}`}
                            right={(props) => <Image
                                {...props}
