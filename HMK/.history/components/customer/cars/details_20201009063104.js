...
async function checkout(Plate,date,{navigation}){
    ...
    if(diffdate){
        try{
                ...
                await firestore().collection('priceset').get().then( price => {
                    price.forEach( async item => {
                       ...
                       await firestore().collection('payment_History').add({
                           ...
                           status:'available',
                           payment_confirm:firestore.Timestamp.fromDate(new Date())
                       }).then(async () =>{
            ...
            Alert.alert('','Done',[{text:'OK',onPress: ({navigation}) => navigation.goBack()}],{ cancelable: false })
        })
        }  
        catch(err){
            console.log(err)
        ...
        <View style={page.container}>
            ...
            <View style={{flexDirection:'row',justifyContent:'space-evenly',margin:5}}> 
                <View style={{justifyContent:'space-evenly'}}>
                    <View    >
                        <Text>ป้ายทะเบียน: {item.Plate}</Text>
                    </View>
                    <Divider style={{borderColor:'black',borderWidth:2}}/>
                    <View>
                        <Text>ยี่ห้อ: {item.Name}</Text>
                    </View>
                </View>
                <View>
                    <Image
            ...
            <View style={{flex:2,flexDirection:'row',padding:5,justifyContent: 'space-evenly',}}>
               ...
               {item.host ? 
                    <Button title="เช็คเอ้าท์" containerStyle={{width:100,alignSelf:'flex-end'}} onPress={() => {checkout(item.Plate,item.expiredate.seconds*1000).then(() => {
                        navigation.goBack()
                    })}}>

                    </Button>
                : <Button title="ลบ" containerStyle={{width:100,alignSelf:'flex-end'}} onPress={() => removeCar(item.Plate,navigation)} >
