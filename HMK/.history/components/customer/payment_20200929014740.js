...
function Payments ({navigation}){
    ...
    const  getPayments = async () => {
            try {
                const id = await AsyncStorage.getItem("ID")
                await firestore().collection('payment_History').where('buyer','==',firestore().doc('customer/'+id)).where('payment_cofirm','<=',firestore.Timestamp.fromDate(new Date())).get().then(snapshot => {
                    if(!snapshot.empty){
                    ...
                    await firestore().collection('payment_History').where('payer','==',firestore().doc('customer/'+id)).where('payment_cofirm','<=',firestore.Timestamp.fromDate(new Date())).get().then(snapshot => {
                        if(!snapshot.empty){
                            snapshot.forEach( async data => {            
                                setPayments(prev => [...prev,data.data()]) 
                            })
