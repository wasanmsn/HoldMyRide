    ...
    return (
        ...
        <View>
            ...
            >
                ...
                {
                data.map((item,index) => 
                        (<Card key={item.UserName} style={{marginVertical:5,flexDirection:'row',justifyContent:'space-between',backgroundColor:'#9AF5F5'}} 
                        ...
                        >
                            <Card.Title title={`ผู้รับฝาก ${item.UserName} `}
                                right={(props) => <Avatar.Image size={50}  style={{backgroundColor:'#9AF5F5'}} source={ img[index] ? {uri: img[index]} : require(defaultImg) } /> }
                            />
                            <Card.Content>
