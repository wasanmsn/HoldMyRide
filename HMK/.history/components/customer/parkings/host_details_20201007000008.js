...
export default function host_details({route,navigation}){
    ...
    const sendRequest = async () =>{
        try {
            ...
            .then(() =>{
                Alert.alert('คำขอส่งไปแล้ว','โปรดรอโฮสต์ยืนยัน',[{text:'OK',onPress: () => navigation.goBack()}],{ cancelable: false })
            })
        } catch (error) {
            console.log(error)
        }
    ...
    const getCars =  async () => {
        ...
        try{
            const id = await AsyncStorage.getItem("ID")
            await firestore().collection('vehicles').where('status','==','available').where('userID','==',id).get().then(snapshot => {
                if(snapshot.empty){
                   Alert.alert('','คุณไม่มีรถที่สามารถฝากได้ตอนนี้',[{text:'OK',onPress: () => navigation.goBack()}],{ cancelable: false })
                }
        ...
        <View style={{flex:1}}>
            ...
            <ScrollView >
                ...
                <View style={styles.service_picker}>
                    <RNPickerSelect
                            placeholder= {  {label: "เลือก บริการ", 
                            value: null, }}                    
                            onValueChange={(value) => setData({...data,Service:value})}
                            items={[{label:"ไม่มี",value:"none"},{label:"Pick Up",value:"pickUp"}]
                            }
                        />
                </View>
                <View style={styles.payment_picker}>
                    <RNPickerSelect
                            placeholder= {  {label: "เลือก การโอน", 
                            value: null, }}
                            onValueChange={(value) => {setData({...data,PaymentMethod:value})
                            }}
                            items={[{label:"ออนไลน์",value:"online"},{label:"ออฟไลน์",value:'offline'}]
                            }
                        />
                </View>
                <View style={styles.back_btnRow}>
                        ...
                        onPress={async () => {
                            ...
                            console.log(data)
                            const allow = await firestore().collection('host').doc(data.ID).get().then((res) => {
                                if(res.data().parkingspace <= res.data().vehicles){
                                    Alert.alert('','ที่จอดเต็มแล้ว',[{text:'OK',onPress: () => navigation.goBack()}],{ cancelable: false })
                                    return
                                }
                                toggleOverlay()
                            }) 
            ...
            <Overlay isVisible={visible} onBackdropPress={toggleOverlay} >
                        <View style={{flex:1,justifyContent:'center',alignItems:'center'}}>
                            <Text style={{color:'green',fontSize:18,alignSelf:'flex-start',marginVertical:10}} >โปรดยืนยันคำขอ</Text>
                            <View style={{height:1,alignSelf:'stretch',borderColor:'blue',borderWidth:2}}></View>
                            <Text style={{color:'#0800FF',marginVertical:5}}>ป่ายทำเบียน : {data.vehicle.data.Plate} {'\n'}
                            ชนิด : {data.vehicle.data.Type}{'\n'}
                            ฝาก : {data.Days} วัน : {data.vehicle.data.Type == "car" ? 100.00*data.Days : 27.00*data.Days} ฿ {'\n'}
                            บริการ : {data.Service} {'\n'}
                            { data.Service == 'ไม่มี' ? null :  `ระยะทาง : ${item.distancematrix.distance.value * 0.001 < price.distance_rate_k ? item.distancematrix.distance.value * 0.001 + 'กิโลเมตร : ' + price.below_distance_rate +'฿' :item.distancematrix.distance.value * 0.001 + 'กิโลเมตร : ' +item.distancematrix.distance.value * 0.001*price.distance_price_k +'฿'} ${'\n'}`}
                            ต้องจ่าย : {data.Total} ฿ {'\n'}
                            วิธีการจ่าย: {data.PaymentMethod} {'\n'}
                             </Text>
                            <View style={{height:1,alignSelf:'stretch',borderColor:'blue',borderWidth:2}}></View>
