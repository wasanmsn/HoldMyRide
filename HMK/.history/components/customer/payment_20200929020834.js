...
function Payments ({navigation}){
    ...
    const  getPayments = async () => {
            try {
                const id = await AsyncStorage.getItem("ID")
                await firestore().collection('payment_History').where('buyer','==',firestore().doc('customer/'+id)).orderBy('payment_cofirm','desc').where('payer','in',[firestore().doc('customer/'+id),null]).get().then(snapshot => {
                    if(!snapshot.empty){
