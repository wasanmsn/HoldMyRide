...
function Payments ({navigation}){
    ...
    const  getPayments = async () => {
            try {
                const id = await AsyncStorage.getItem("ID")
                const pays = firestore().collection('payment_History').orderBy('payment_cofirm')
                await pays.where('buyer','==',firestore().doc('customer/'+id)).get().then(snapshot => {
                    if(!snapshot.empty){
                    ...
                    await pays.where('payer','==',firestore().doc('customer/'+id)).get().then(snapshot => {
                        if(!snapshot.empty){
                            snapshot.forEach( async data => {            
                                setPayments(prev => [...prev,data.data()]) 
                            })
