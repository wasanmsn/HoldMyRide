...
function Cars ({route,navigation}){
    ...
    const MyCard = props =>{ 
            const [backgroundColor,setbackgroundColor] = React.useState('#1ca6d4')
            const [borderColor,setborderColor] = React.useState('#128cb5')
            return (
            ...
            >
                <Card.Title
                            ...
                            subtitle={`${item.expiredate == null ? '': "เหลือเวลาอีก "+Math.floor((new Date(props.data.expiredate.seconds*1000).getTime() - new Date().getTime())/(1000*3600*24))+' วัน'}`}
