...
function details({route,navigation}){
    const {array,name,imge,id,days} = route.params
    return(
        <View style={page.container}>
            ...
            <View style={{padding:5}} >
                <Text>
                    Check in {new Date(array.expiredate.seconds*1000).toLocaleDateString()} {'\n'}
                    Expires in {days} days {new Date().setDate(new Date().getDate()+parseInt(days))}
                </Text>
            </View>
            <View style={{flex:2,flexDirection:'row',padding:5,justifyContent: 'space-evenly',}}>
                    ...
                    <Button title="CHECKOUT" containerStyle={{width:100,alignSelf:'flex-end'}} onPress={() => {checkout(id)}}>
