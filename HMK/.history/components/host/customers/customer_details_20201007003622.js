    ...
    const acceptRequest = async () =>{
        try {
            ...
            await firestore().collection('payment_History').add({
                ...
                Alert.alert('คำขอยอมรับแล้ว','',[{text:'OK',onPress: () => navigation.navigate('Home')}],{ cancelable: false })
        ...
        <View style={{flex:1}}>
            ...
            <ScrollView style={{height:height*0.6}}>
                <Card style={styles.nAMEBOX}>
                        <Card.Title title={request.name ?"ลูกค้า "+ request.name : "This is test name"} />
                 </Card>
                 <View >
                        <MapView
                                    initialRegion={{
                    ...
                    <Card style={{height:170,alignSelf:'stretch',borderColor:'black',borderWidth:2}}>
                        <Card.title title={"พาหนะ"} />
                        <Card.Content>
                            <Text style={styles.nameName} >พาหนะ</Text>
                            <Text style={styles.nameName} >ป้ายทะเบียน : {vehicels.data.Plate} {'\n'} ชนิด : {vehicels.data.Type.toUpperCase()} </Text>
                            <View style={{margin:5}}>
                    ...
                    <View style={{height:140,alignSelf:'stretch',borderColor:'black',borderWidth:2}}>
                        <Text style={styles.nameName} >ฝาก {request.days} วัน </Text>
                        <Text style={styles.nameName} >บริการรับถึงที่: {request.service == 'pickUp' ? 'Yes' : 'No'} </Text>
                        <Text style={styles.nameName} >การจ่าย: {request.payment_method.toUpperCase()} </Text>
                    </View>
