...
export default function customers({navigation}){
    ...
    const getReqs = async () => {
        try {
            ...
            const obsData = await data.onSnapshot(async querySnapshots => {
                console.log(querySnapshots.size)
                
               await querySnapshots.docChanges().forEach(async change => {
                    if(change){
                        setRequest([])
                        
                    }
                    const docs = await querySnapshots.docs.map(doc => {
