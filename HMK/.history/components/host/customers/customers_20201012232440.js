...
import { Text,ScrollView,Dimensions,RefreshControl } from 'react-native';
...
export default function customers({navigation}){
    ...
    const getFirstData = async () => {
        try {
                ...
                const data = await firestore().collection('requests').where('host','==',firestore().doc('host/'+ID)).where('status','==',false).get().then(res => {
                    console.log('wtf',res.size)
                        res.forEach(doc => {
                            reqs.push({data:doc.data(),id:doc.id})
                        })
                        setRequest(reqs)
                    })
    ...
    return(
        <LinearGradient colors={['#1ca7ec','#787ff6']} start={{x: 0, y: 0}} end={{x: 1, y: 1}} style={{height:height}}>
            <Header />
            <ScrollView style={{height:height*0.8}} refreshControl={
