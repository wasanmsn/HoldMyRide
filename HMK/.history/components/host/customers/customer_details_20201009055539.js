...
import Linergrad from 'react-native-linear-gradient'
...
export default function customer_detail({route,navigation}){
    ...
    return(
        <Linergrad colors={['#78d5f5','#787ff6','4adede']} start={{x: 0, y: 0}} end={{x: 1, y: 1}} style={{flex:1}}>
            <Header />
                ...
                <Card style={styles.nAMEBOX}>
                 ...
                 <View >
                    ...
                    <Card style={{alignSelf:'stretch',borderColor:'#78d5f5',borderWidth:2,backgroundColor:'#1Ca7ec'}} onPress={toggleOverlay} >
                        <Card.Title titleStyle={{color:'#4ADEDE'}} title={"พาหนะ"} right={(props) =><Image
                            {...props}
                            ...
                            />
                        <Card.Content>
                            <Text style={{color:'#1f2f98'}} >ป้ายทะเบียน : {vehicels.data.Plate} {'\n'}ชนิด : {vehicels.data.Type.toUpperCase()} </Text>
                            <Text style={{color:'#1f2f98'}} >ฝาก {request.days} วัน </Text>
                            <Text style={{color:'#1f2f98'}} >บริการรับถึงที่ {request.service == 'pickUp' ? 'Yes' : 'No'} </Text>
                            <Text style={{color:'#1f2f98'}} >การจ่าย {request.payment_method.toUpperCase()} </Text>
                        </Card.Content>
           ...
           <Overlay isVisible={visible} onBackdropPress={toggleOverlay} overlayStyle={{width:250,height:250,backgroundColor:'#1ca7ec'}} >
                        <View>
                        <Image
                            source={ img ? {uri: img} : require(defaultImg) }
