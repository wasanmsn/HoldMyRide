                ...
                await firestore().collection('payment_History').where('saler','==',firestore().doc('host/'+id)).get().then(snapshot => {
                    if(!snapshot.empty){
                        snapshot.forEach( async data => {       
                            console.log('id: ',data.id,' date: ',data.data().payment_cofirm ? new Date(data.data().payment_cofirm.seconds*1000).toLocaleString() : 'no')     
                    ...
                    await firestore().collection('payment_History').where('payer','==',firestore().doc('host/'+id)).get().then(snapshot => {
                        ...
                        arrays.forEach(element => {
                            ...
                            console.log('data ',element.payment_cofirm ? new Date(element.payment_cofirm.seconds*1000).toLocaleString() : 'no')
                        });
