...
async function removeCar(Plate,userID,navigation){
    var PlateId=''
    await firestore().collection('vehicles').where("Plate","==",Plate).get().then( docs => {
        docs.forEach( async doc => {
            var pic = doc.data().pic
            var name = pic.slice(-17)
            var carPicRef = firebase.storage().ref().child(`carPics/${userID}/${name}`)
        ...
        <View style={page.container}>
            ...
            <View style={{flex:2,flexDirection:'row',padding:5,justifyContent: 'space-evenly',}}>
               ...
               {item.host ? 
                ...
                : <Button title="REMOVE" containerStyle={{width:100,alignSelf:'flex-end'}} onPress={() => removeCar(item.Plate,useID,navigation)} >
