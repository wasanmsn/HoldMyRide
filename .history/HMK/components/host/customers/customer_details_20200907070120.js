    ...
    const getPayment =  async () => {
        try{
            ...
            const vehicel  = { data : await payment_data.vehicel.get().then(res => res.data()),
                                ...
                                img : await payment_data.vehicel.get().then(async res => {await firestorage().refFromURL(res.data().pic).getDownloadURL()})
            }
        ...
        <View style={{flex:1}}>
            ...
            <ScrollView>
                <View style={styles.nAMEBOX}>
                    ...
                    <View style={{height:150,alignSelf:'stretch',borderColor:'black',borderWidth:5}}>
                        <Text style={styles.nameName} >Vehicle</Text>
                        <Text style={styles.nameName} >Plate number : {vehicels.data.Plate} Type : {vehicels.data.Type} </Text>
                        <View>
                            <Image
                            
                            source={ vehicels.img ? {uri: vehicels.img} : require(defaultImg) }
