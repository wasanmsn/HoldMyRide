    ...
    const acceptRequest = async () =>{
        try {
            ...
            await firestore().collection('payment_History').add({
                ...
                await firestore().collection('wallet').where('owner','==',firestore().doc('host/'+request.host)).get().then(  docs => {
                    docs.forEach( async doc => {
                        const newBal = doc.data().balance + request.total
                        await firestore().collection('wallet').doc(doc.id).update({balance: newBal})
                    })     
                }).then(async() => {
                    await firestore().collection('wallet').where('owner','==',firestore().doc('host/'+request.client)).get().then(  docs => {
                        docs.forEach(async doc => {
                            const newBal = doc.data().balance - request.total
