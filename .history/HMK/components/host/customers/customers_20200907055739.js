                    ...
                    requests.map((item,index) => (<TouchableOpacity key={index} style={{borderColor:'black',borderWidth:1,height:70,marginVertical:5,flexDirection:'row',justifyContent:'space-between'}} 
                        onPress={ async () => {
                            navigation.navigate(`Customers Detail`,{
                                request:{
                                    client:await item.data.client.get().then(res => res.data()),
                                    client_id : await item.data.client.get().then(res => res.id),
                                    date :item.data.date,
                                    days :item.data.days,
                                    distance :item.data.distance,
                                    host : await item.data.host.get().then(res => res.data()),
                                    host_id : await item.data.host.get().then(res => res.id),
                                    coordinates :item.data.coordinates,
                                    name : item.data.name,
                                    payment : await item.data.payment.get().then(res => res.data()),
                                    payment_id : await item.data.payment.get().then(res => res.id),
