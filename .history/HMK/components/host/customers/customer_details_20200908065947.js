        ...
        try {
            ...
            await firestore().collection('payment_History').add({
                ...
                await firestore().collection('vehicles').doc(request.vehicle).update({
                    ...
                    parkingAt:request.parkingAt
                
                })
            }).then(async() => {
                await firestore().collection('wallet').where('owner','==',firestore().doc('host/'+request.host)).get().then( async docs => {
                    docs.forEach(doc => {
                        const newBal = doc.data().balance + request.total
                        await firestore().collection('wallet').doc(doc.id).update({balance: newBal})
                    })     
                }).then(() => {
                    await firestore().collection('wallet').where('owner','==',firestore().doc('host/'+request.client)).get().then( async docs => {
                        docs.forEach(doc => {
                            const newBal = doc.data().balance - request.total
                            await firestore().collection('wallet').doc(doc.id).update({balance: newBal < 0 ? 0 :  newBal })
                        })
                    })
                })
            }).then(async() => {
