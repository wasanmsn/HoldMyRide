...
export default function customer_detail({route,navigation}){
    ...
    const getVehicle = async () => {
        try{
            var id = ''
            const vehicel_data = await data.vehicel.get().then(res => res.data() ).then(res => id = res.id)
            
            console.log('hello')
            return {data:vehicel_data,id:id}
        }
        catch(err) {
            console.log(err)
        }
    ...
    return(
        <View style={{flex:1}}>
            <Header />
            <Text>
                Payment : {payment.days} Vehi : {vehicels.data.Name}
            </Text>
        </View>
    )
}
