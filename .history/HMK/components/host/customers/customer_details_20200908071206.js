        ...
        try {
            ...
            await firestore().collection('payment_History').add({
                ...
                await firestore().collection('host').doc(request.host).get().then( async  doc => {
                     const id = await doc.data().wallet.get().then(res => res.id)
                     const newBal = doc.data().balance + request.total
                     await firestore().collection('wallet').doc(id).update({balance: newBal})
                })
                await firestore().collection('customer').doc(request.host).get().then( async  doc => {
                    const id = await doc.data().wallet.get().then(res => res.id)
                    const newBal = doc.data().balance - request.total
                    await firestore().collection('wallet').doc(id).update({balance: newBal < 0 ? 0 : newBal})
               })
            }).then(async() => {
