        ...
        try {
            ...
            await firestore().collection('payment_History').add({
                ...
                await firestore().collection('customer').doc(request.client).get().then( async  doc => {
                    const id = await doc.data().wallet.get().then(res => res.id)
                    const balance = await firestore().collection('wallet').doc(id).get().then(res => res.data().balance)
                    const newBal = balance - request.total
                    await firestore().collection('wallet').doc(id).update({balance: firebase.firestore.FieldValue.increment(newBal < 0 ? 0 : newBal)})
               })
            }).then(async() => {
