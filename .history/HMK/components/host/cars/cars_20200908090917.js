...
function Cars (){
    ...
    const showVehicles = async () => {
        var start = 0
            start = currentSet.start
        var end = currentSet.end

        for (start; start < 2; start++) {
            console.log(vehicles[1]._data.expiredate.seconds*1000)
            console.log(start)
            const name = await vehicles[start]._data.owner.get().then(res => res.data().Name)
            const date = Math.floor((new Date(vehicles[start].data.expiredate.seconds*1000).getTime() - new Date().getTime())/(1000*3600*24))
            setVehicle(prev => [...prev,
                <TouchableOpacity style={{borderColor:'black',borderWidth:1,height:85,marginVertical:15,flexDirection:'row',justifyContent:'space-evenly'}} >
                     <View style={{marginVertical:15,right:20}}>
                            <Text>
                            Car Plate number {vehicles[start]._data.Plate} {'\n'}
                            Car manufacturer {vehicles[start]._data.Plate} {'\n'}
