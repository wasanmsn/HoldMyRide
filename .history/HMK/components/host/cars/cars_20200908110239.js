...
function Cars (){
        ...
        try {
            ...
            const obsData =  data.onSnapshot(querySnapshots => {
                    ...
                    querySnapshots.docChanges().forEach(change => {
                        if(vehicles.length > querySnapshots.size || vehiclesSet.length > querySnapshots.size){
                            console.log(vehiclesSet.length)
                            setVehicles([])
                            setVehicle([])
                        }
                        if (change.type === 'added') {
                          console.log('New city: ', change.doc.data());
                          setVehicles(prev => [...prev,change.doc.data()])
    ...
    const showVehicles = async () => {
        setVehicle([])
        if(vehicles.length > length || vehiclesSet.length > length){
            console.log(vehiclesSet.length)
            setVehicles([])
            setVehicle([])
        }
