            ...
            <View style={page.box1}>
                <ScrollView>
                    <View  >
                        {vehicles.map( (data) => 
                            (
                             <TouchableOpacity key={`${data._data.Plate}`} style={{borderColor:'black',borderWidth:1,height:85,marginVertical:15,flexDirection:'row',justifyContent:'space-evenly'}} >
                             <View style={{marginVertical:15,right:20}}>
                                    <Text>
                                    Car Plate number {data._data.Plate} {'\n'}
                                    Car manufacturer {data._data.Name} {'\n'}
                                    Customer's name  { data._data.owner.get().then(res => res.data().Name)}
                                    </Text>
                                </View>
                                <View style={{marginVertical:15,right:20}}>
                                    <Text>
