        ...
        try {
            ...
            const obsData =  data.onSnapshot(querySnapshots => {
                    ...
                    querySnapshots.docChanges().forEach(async change => {
                            ...
                            console.log("Chaged")
                            const Name = await change.doc.data().owner.get().then(res => setName(prev => [...prev,res.data().Name]))
                            const Imge =  await firestorage().refFromURL(change.doc.data().pic).getDownloadURL().then(res => setImge(prev => [...prev,res]))
