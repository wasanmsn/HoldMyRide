            ...
            const obsData =  data.onSnapshot(querySnapshots => {
                    ...
                    querySnapshots.docChanges().forEach(async change => {
                         ...
                         datas.forEach(async data => {
                            var name = await data.owner.get().then(res => res.data().Name)
                            var img = await firestorage().refFromURL(data.pic).getDownloadURL()
                            setName(prev => [...prev,name])
                            setImge(imge => [...imge,img])
                         })
                         setVehicles(datas)
            },err => {
                console.log(err)
                    ...
                    <View  >
                        {vehicles.map((array,index) => 
                            ...
                            <View style={{marginVertical:15,right:20}}>
                               ...
                               <View style={{marginVertical:15,right:20}}>
                                   <Text>
                                       {Math.ceil(Math.abs(new Date(array.expiredate.seconds*1000) - new Date()))/(1000 * 60 * 60 * 24).toPrecision(21)}{'\n'}
