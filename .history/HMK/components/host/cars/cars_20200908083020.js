...
function Cars (){
        ...
        try {
            ...
            const obsData = data.onSnapshot(querySnapshots => {
                ...
                querySnapshots.docChanges().forEach(change => {
                    ...
                    const docs = querySnapshots.docs.map(async doc => {
                        await firestorage().refFromURL(doc.data().pic).getDownloadURL().then((res) =>{
                            setimge(prev => [...prev,res])
                        })
                        setVehicles(async prev => [...prev,{data:doc.data(),id:doc.id,name:await doc.data().owner.get().then(res => res.data().Name)}])
                    })
                })
            },err => {
                console.log(err)
    ...
    const showVehicles = () => {
        ...
        for (currentSet.start; currentSet.start < currentSet.end; setCurrentSet({...currentSet,start:currentSet.start+1})) {
