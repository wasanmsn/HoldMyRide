            ...
            <View style={page.box1}>
                <ScrollView>
                    <View  >
                        {vehicles.map( (item) => 
                            (
                             <TouchableOpacity key={`${item._data.Plate}`} style={{borderColor:'black',borderWidth:1,height:85,marginVertical:15,flexDirection:'row',justifyContent:'space-evenly'}} >
                             <View style={{marginVertical:15,right:20}}>
                                    <Text>
                                    Car Plate number {item._data.Plate} {'\n'}
                                    Car manufacturer {item._data.Name} {'\n'}
                                    Customer's name  { item._data.owner.get().then(res => res.data().Name)}
                                    </Text>
                                </View>
                                <View style={{marginVertical:15,right:20}}>
                                    <Text>
                                        {Math.floor((new Date(item._data.expiredate.seconds*1000).getTime() - new Date().getTime())/(1000*3600*24))} Days left
                                    </Text>
                                </View>
                                {/* <View style={{marginVertical:15,right:20}}>
                                    <Image
