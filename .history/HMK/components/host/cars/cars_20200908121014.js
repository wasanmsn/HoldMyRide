...
function Cars (){
    ...
    const getNameandImge = async (owner,imge) =>{
        var name = await owner.get().then(res => res.data().Name)
        var img = await firestorage().refFromURL(imge).getDownloadURL()
        return {name,imge}
    }
    const getDatas =  (owner,imge) =>{
            getNameandImge(owner,imge).then(data => {
                setName(preve => [...preve,data.name])
                setImge(preve => [...preve,data.img])
                    ...
                    <View  >
                        {vehicles.map(array => 
                            ...
                            <View style={{marginVertical:15,right:20}}>
                                   <Text>
                                       {getDatas(array.owner,array.imge)}
