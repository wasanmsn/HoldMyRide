                ...
                <ScrollView>
                    <View  >
                        {vehicles.map(async (data,index) => {
                            const name = await data._data.owner.get().then(res => res.data().Name)
                            const date =  Math.floor((new Date(data._data.expiredate.seconds*1000).getTime() - new Date().getTime())/(1000*3600*24))
                            (
