...
import googleDistance from 'react-native-google-matrix'
                ...
                await query.get().then(val => {
                    val.docs.forEach(id => {
                        firestore().collection('host').doc(id.id).get().then( data => {
                            googleDistance.apiKey = GOOGLE_MAPS_APIKEY
                            googleDistance.get({
                                origin: toString(info.coords.latitude)+','+toString(info.coords.longitude),
                                destination: toString(data.data().houselocation._latitude)+','+toString(data.data().houselocation._longitude),
                                units:'metric'
                            },(err,data) => {
                                if (err) return
                                console.log(data)
