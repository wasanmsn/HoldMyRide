...
async function getCar(){
    ...
    try{
        ...
        await firestore().collection('vehicles').where('userID','==',id).get().then(snapshot => {
            ...
            snapshot.forEach( vehicle => {
                    vehi.push({Data:vehicle.data(),ID:vehicle.id})
    ...
    return vehi.map(item => {
        (<Picker.Item key={item.Data.Plate} label={item.Data.Plate + ` ` + item.Data.Name} value={item} />)
    })
}
    ...
    const getCars =  async () => {
        try{
            ...
            await firestore().collection('vehicles').where('userID','==',id).get().then(snapshot => {
                ...
                snapshot.forEach( vehicle => {
                        setVehi( prev => [...prev,{Data:vehicle.data(),ID:vehicle.id}])
