...
function fetchDistance(origin,desti){
    return new Promise((resolve, reject) => {
        const url = `https://maps.googleapis.com/maps/api/distancematrix/json?units=metric&origins=${origin.latitude},${origin.longitude}&destinations=${desti._latitude},${desti._longitude}&key=${GOOGLE_MAPS_APIKEY}`
        console.log(url)
                ...
                await query.get().then(val => {
                    val.docs.forEach(id => {
                        firestore().collection('host').doc(id.id).get().then( data => {
                            ...
                            fetchDistance(info.coords,data.data().houseLocation).then(res => {
                                console.log(res)
                            })
                        })
                    })
