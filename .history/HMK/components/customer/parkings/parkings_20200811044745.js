                    ...
                    val.docs.forEach(id => {
                        firestore().collection('host').doc(id.id).get().then( data => {
                            ...
                            axios.get(url).then( res => {
                                ...
                                const datas = {...data.data(),distancematrix}
        ...
        <View>
            ...
            <ScrollView style={styles.box1}>
                ...
                data.map((item,index) => 
                    ...
                    (<TouchableOpacity key={item.UserName} style={{borderColor:'black',borderWidth:1,height:150,marginVertical:5,flexDirection:'row',justifyContent:'space-between'}} 
                    >
                     <View    style={{marginVertical:5,padding:2}}>
                            <Text>
                            Host name : {item.UserName} {'\n'}
                            Distance : {item.distancematrix.distance} {'\n'}
                            Locate : {item.distancematrix.location}
                            </Text>
                        </View>
